...
# Aggregate intensities for each protein and each sample
sapply(abrfprots, function(x){ 
  test <- maxquant %>% select(Raw.file, Proteins, Intensities) %>%
    filter(grepl(x, Proteins)) %>%
    group_by(Raw.file) %>%
    summarise(intensity = sum(as.numeric(unlist(strsplit(Intensities, ';')))))
  print(test)
  })
```
...
# Aggregate intensities for each protein and each sample
sapply(abrfprots, function(x){ 
  test <- maxquant_mbr %>% select(Raw.file, Proteins, Intensities) %>%
    filter(grepl(x, Proteins)) %>%
    group_by(Raw.file) %>%
    summarise(intensity = sum(as.numeric(unlist(strsplit(Intensities, ';')))))
  print(test)
