...
moff_files <- list.files('2019_datasets/ABRF/moFF_SGPS_Galaxy/moFFnoMBR/peptide_summary/',
                    full.names = TRUE)
...
### With match-between runs (with MBR)
```{r}
moff <- read.delim("2019_datasets/ABRF/moFF_SGPS_Galaxy/moFFMBR/Galaxy9-[moFF_on_data_1,_data_2,_and_others__peptide_summary].tabular", stringsAsFactors = FALSE)

# Aggregate intensities for each protein and each sample
sapply(abrfprots, function(x){ 
    moff %>% filter(grepl(x, prot)) %>%
    group_by(prot) %>%
    summarise_if(is.numeric, sum)
  })
```
