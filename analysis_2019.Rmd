...
```{r flash}
flash <- read.delim("2019_datasets/FLASH_SGPS_Galaxy/flashMBR/Galaxy148-[FlashLFQ_on_data_141,_data_140,_and_others__QuantifiedPeptides.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim("2019_datasets/FLASH_SGPS_Galaxy/flashnoMBR/Galaxy144-[FlashLFQ_on_data_141,_data_140,_and_others__QuantifiedPeptides.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
...
### moFF
```{r moff}
moff <- read.delim("2019_datasets/moFF_SGPS_Galaxy/moFFMBR/Galaxy9-[moFF_on_data_1,_data_2,_and_others__peptide_summary].tabular",
                   stringsAsFactors = FALSE)
...
```{r moff_no_mbr}
# moff, no mbr
moff_files <- list.files('2019_datasets/moFF_SGPS_Galaxy/moFFnoMBR/peptide_summary/',
                    full.names = TRUE)
...
#### Peptides
```{r mqpep}
mq <- read.delim("2019_datasets/MaxQuant_GUI/with_MBR/msms.txt",
                 stringsAsFactors = FALSE) %>%
...
# mq no mbr
mq_no_mbr <- read.delim("2019_datasets/MaxQuant_GUI/no_MBR/msms.txt",
                        stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Proteins) %>%
    select(peptide, prot, starts_with("Intensity."))
...
# mq prot
mq_data <- mq_prot_analysis("UPS/MQ_1pep/proteinGroups.txt", "MaxQuant")

# mq prot, no mbr
mq_no_mbr_data <- mq_prot_analysis("UPS/MQ_1pep_noMBR/proteinGroups.txt", "MaxQuant_no_MBR")
```
...
fc_accuracy <- rbind(all_df, mq_data$rmlse, mq_no_mbr_data$rmlse) %>%
    rename("Normalization" = Var1,
           "Tool" = Var2) 
write.table(fc_accuracy, file = "fold_change_accuracy.tabular",
            quote = FALSE,
            row.names = FALSE,
            sep = '\t')
```
...
write.table(stat_table, file="differential_expression_results.tab",
...
```{r}
rmsles_hi <- rmsles_low <- n_low <- n_hi <- rep(0, nrow(method_list))
...
mq_low <- filter(mq_data$joined_mq, ratios < 1)
...
# low
mq_no_mbr_low <- filter(mq_no_mbr_data$joined_mq, ratios < 1)
...
hilo <- cbind(method_list, "high" = rmsles_hi, "nhi" = n_hi,
...
all_hilo <- rbind(hilo, mq_mbr, mq_no_mbr) %>%
    rename("Normalization" = Var1, "Tool" = Var2)

write.table(all_hilo, file="high_low_fold_change_accuracy_new.tab",
            sep="\t", quote=FALSE, row.names=FALSE)
``` 
