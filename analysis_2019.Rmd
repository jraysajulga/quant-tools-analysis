...
```{r flash}
flash <- read.delim("UPS/FlashLFQ_UPS/msms_FlashLFQ_QuantifiedBaseSequences.tsv",
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim("UPS/flash_noMbr/msms_FlashLFQ_QuantifiedBaseSequences.tsv",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
