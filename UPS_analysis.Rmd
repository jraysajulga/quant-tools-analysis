...
```{r}
root <- "Quant_Paper_dec_results"
get_filename <- function(directory, regex){
  directory <- paste0(root, directory)
  paste0(directory, list.files(directory, pattern=regex))
}
```
...
```{r flash}
flash <- read.delim(get_filename("/UPS/FLASHLFQ/MBR\ without\ normalization/", ".+QuantifiedPeptides.tsv].tabular"),
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim(get_filename("/UPS/FLASHLFQ/noMBR\ without\ normalization/", ".+QuantifiedPeptides.tsv].tabular"),
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
...
```{r flash}
flash_norm <- read.delim(get_filename("/UPS/FLASHLFQ/MBR\ with\ normalization/",
                                      ".+QuantifiedProteins.tsv].tabular"),
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr_norm <- read.delim(get_filename("/UPS/FLASHLFQ/noMBRwith\ normalization/",
                                             ".+QuantifiedProteins.tsv].tabular"),
                           stringsAsFactors = FALSE) %>%
    rename(prot = Protein.Groups) %>%
    select(Gene.Name, prot, starts_with("Intensity"))
...
### moFF
```{r moff}
moff <- read.delim(get_filename("/UPS/moff/MBR\ without\ normalization/", ".+peptide_summary].tabular"),
                   stringsAsFactors = FALSE)
...
```{r moff_no_mbr}
# moff, no mbr
moff_files <- list.files(paste0(root, '/UPS/moff/noMBR\ without\ normalization/peptide\ summary/'),
                    full.names = TRUE)
