...
take_first_protein <- function(prot_vec){
    str_split(str_split(prot_vec, ",", simplify = TRUE)[, 1], ";", simplify = TRUE)[, 1]
}
```
...
```{r flash}
flash <- read.delim("Quant\ Paper\ Redone_11102019/UPS/FLASHLFQ/MBR\ without\ normalization/Galaxy214-[FlashLFQ_on_data_20,_data_18,_and_others__QuantifiedPeptides.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim("Quant\ Paper\ Redone_11102019/UPS/FLASHLFQ/noMBR\ without\ normalization/Galaxy226-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedPeptides.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
...
```{r flash}
flash_norm <- read.delim("Quant\ Paper\ Redone_11102019/UPS/FLASHLFQ/MBR\ with\ normalization/Galaxy100-[FlashLFQ_on_data_20,_data_18,_and_others__QuantifiedPeptides.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
flash$prot <- take_first_protein(flash$prot)
...
# flash no mbr
flash_no_mbr_norm <- read.delim("Quant\ Paper\ Redone_11102019/UPS/FLASHLFQ/noMBRwith\ normalization/Galaxy106-[FlashLFQ_on_data_20,_data_18,_and_others__QuantifiedPeptides.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
flash_no_mbr$prot <- take_first_protein(flash_no_mbr$prot)
...
### moFF
```{r moff}
moff <- read.delim("Quant\ Paper\ Redone_11102019/UPS/moff/MBR\ without\ normalization/Galaxy137-[moFF_on_data_133,_data_132,_and_others__peptide_summary].tabular",
                   stringsAsFactors = FALSE)
...
```{r moff_no_mbr}
# moff, no mbr
moff_files <- list.files('Quant\ Paper\ Redone_11102019/UPS/moff/noMBR\ without\ normalization/moFF\ on\ collection\ 134:\ peptide\ summary',
                    full.names = TRUE)
...
moff_no_mbr <- lapply(moff_files, function(i) df = read.delim(i, as.is = TRUE) %>% 
  rename(!!paste("sumIntensity_20130510_EXQ1_IgPa_QC_",
                 str_extract(i, "UPS[0-9_]+"), sep="") := sumIntensity_UPS_PSM_All)) %>%
...
#### Peptides
```{r mqpep}
mq <- read.delim("Quant\ Paper\ Redone_11102019/UPS/MAXQUANT/UPS_MBR/combined/txt/peptides.txt",
                 stringsAsFactors = FALSE) %>%
...
# mq no mbr
mq_no_mbr <- read.delim("Quant\ Paper\ Redone_11102019/UPS/MAXQUANT/UPS_noMBR/combined/txt/peptides.txt",
                        stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Proteins) %>%
    select(peptide, prot, starts_with("Intensity."))
...
# mq prot
mq_data <- mq_prot_analysis("Quant\ Paper\ Redone_11102019/UPS/MAXQUANT/UPS_MBR/combined/txt/proteinGroups.txt", "MaxQuant")

# mq prot, no mbr
mq_no_mbr_data <- mq_prot_analysis("Quant\ Paper\ Redone_11102019/UPS/MAXQUANT/UPS_noMBR/combined/txt/proteinGroups.txt", "MaxQuant_no_MBR")
```
...
# MaxQuant
```{r}
mq_msms <- read.delim("2019_datasets/UPS/MQ_1pep/msms.txt", stringsAsFactors = FALSE)
length(unique(mq$peptide)) 
length(unique(mq_msms$Sequence))
setdiff(unique(mq$peptide) , unique(mq_msms$Sequence))
```

# PSM Report
```{r}
psm_rep <- read.delim("2019_datasets/UPS/psm-report_Galaxy60-[Column_Regex_Find_And_Replace_on_data_50].tabular", stringsAsFactors = FALSE)
length(unique(psm_rep$Sequence))
```

# moFF
```{r}
length(unique(moff$peptide))
```

# flashLFQ
```{r}
length(unique(flash$Sequence))
```

## Write peptide tables out

```{r}
write.table(unique(mq$peptide), file="peptides/UPS/mq_peptides.tab",
            sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
write.table(unique(moff$peptide), file="peptides/UPS/moff_peptides.tab",
            sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
write.table(unique(flash$peptide), file="peptides/UPS/flash_peptides.tab",
            sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
```
