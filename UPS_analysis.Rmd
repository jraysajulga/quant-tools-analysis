...
```{r}
get_protein <- function(prot_vec){
    sapply(prot_vec, function ( row ) {
      delimiter <- unlist(sapply(c(', ', ';'),
                                 function(delimiter) {
                                   if (length(grep(delimiter, row)) > 0){ delimiter }
                                 }))[[1]]
      if (length(delimiter) > 0){
        proteins <- str_split(row, delimiter, simplify = TRUE)
        results <- proteins[grepl("ups", proteins)]
        if (length(results) == 0){
          proteins[1]
        } else {
          results[1]
...
```{r flash}
flash <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/MBR\ without\ normalization/Galaxy240-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedPeptides.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/noMBR\ without\ normalization/Galaxy252-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedPeptides.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
...
```{r flash}
flash_norm <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/MBR\ with\ normalization/Galaxy235-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedProteins.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
    rename(prot = Protein.Groups) %>%
    select(Gene.Name, prot, starts_with("Intensity"))
flash$prot <- get_protein(flash$prot)
#flash_names <- names(flash)

# flash no mbr
flash_no_mbr_norm <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/noMBRwith\ normalization/Galaxy247-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedProteins.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(prot = Protein.Groups) %>%
    select(Gene.Name, prot, starts_with("Intensity"))
flash_no_mbr$prot <- get_protein(flash_no_mbr$prot)
...
### moFF
```{r moff}
moff <- read.delim("Quant_Paper_dec_results/UPS/moff/MBR\ without\ normalization/Galaxy137-[moFF_on_data_133,_data_132,_and_others__peptide_summary].tabular",
                   stringsAsFactors = FALSE)
...
```{r moff_no_mbr}
# moff, no mbr
moff_files <- list.files('Quant_Paper_dec_results/UPS/moff/noMBR\ without\ normalization/peptide\ summary/',
                    full.names = TRUE)
...
peptides_normalize_and_test_de <- function(df,
    ...
    df_norm <- data.frame(prot = df$prot,
                          intensities,
                          stringsAsFactors = FALSE)
