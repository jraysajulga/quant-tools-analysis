...
```{r flash}
flash_norm <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/MBR\ with\ normalization/Galaxy234-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedPeptides.tsv].tabular",
                    stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
flash$prot <- take_first_protein(flash$prot)
...
# flash no mbr
flash_no_mbr_norm <- read.delim("Quant_Paper_dec_results/UPS/FLASHLFQ/noMBRwith\ normalization/Galaxy246-[FlashLFQ_on_data_20,_data_133,_and_others__QuantifiedPeptides.tsv].tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
flash_no_mbr$prot <- take_first_protein(flash_no_mbr$prot)
