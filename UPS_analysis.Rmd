...
```{r flash}
flash <- read.delim("2019_datasets-072919/UPS/FlashLFQ_UPS/Galaxy104-[FlashLFQ_on_data_39,_data_38,_and_others__QuantifiedPeptides.tsv]-081319.tabular",
                    stringsAsFactors = FALSE) %>%
...
# flash no mbr
flash_no_mbr <- read.delim("2019_datasets-072919/UPS/flash_noMbr/Galaxy100-[FlashLFQ_on_data_39,_data_38,_and_others__QuantifiedPeptides.tsv]-081319.tabular",
                           stringsAsFactors = FALSE) %>%
    rename(peptide = Sequence, prot = Protein.Groups) %>%
    select(peptide, prot, starts_with("Intensity"))
...
### moFF
```{r moff}
moff <- read.delim("2019_datasets-072919/UPS/moff/Galaxy13-[moFF_on_data_1,_data_2,_and_others__peptide_summary].tabular",
                   stringsAsFactors = FALSE)
...
```{r moff_no_mbr}
# moff, no mbr
moff_files <- list.files('2019_datasets-072919/UPS/moff_noMbr/peptide\ summary/',
                    full.names = TRUE)
...
moff_no_mbr <- lapply(moff_files, function(i) df = read.delim(i, as.is = TRUE) %>% 
  rename(!!paste("sumIntensity_20130510_EXQ1_IgPa_QC_",
                 str_extract(i, "UPS[0-9_]+"), sep="") := sumIntensity_Galaxy41.__ob__UPS_PSM__cb__)) %>%
